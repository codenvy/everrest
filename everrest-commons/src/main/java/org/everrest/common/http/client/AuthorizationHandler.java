...
/**
 * This is the interface that an Authorization handler must implement. You can
 * implement your own auth handler to add support for auth schemes other than
...
public interface AuthorizationHandler
{
    ...
    * candidate info is found in the list of known auth info. Usually this method
    * will query the user for the necessary info.
    * <P>
    ...
    * @return the authorization info to use when retrying the request, or null if
    *         the request is not to be retried. The necessary info includes the
    *         host, port, scheme and realm as given in the <var>challenge</var>
    *         parameter, plus either the basic cookie or any necessary params.
    * @exception AuthSchemeNotImplException if the authorization scheme in the
    ...
    * info in the AuthorizationInfo class to be sent with a request. This happens
    * when either auth info is being preemptively sent or if a 401 response is
    * retrieved and a matching info is found in the list of known info. The
    ...
    *          this is in response to a 401, or null if we are preemptively
    *          sending the info.
    * @param resp the full 401 response received, or null if we are preemptively
    *          sending the info.
    * @return the authorization info to be sent with the request, or null if none
    *         is to be sent.
    * @exception AuthSchemeNotImplException if the authorization scheme in the
    *              info cannot be handled.
    * @exception IOException if an exception occurs while fixing up the info
    */
    ...
    * authorization (such as the "Authentication-Info" header). Therefore this
    * method is invoked for each response received, even if it is not a 401 or
    ...
    * @exception IOException if an exception occurs during the reading of the
    *              headers.
    */
