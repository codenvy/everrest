         ...
         try
         {
            synchronized (re.first)
            {
               ...
               for (RetryException e = re.first; e != null; e = e.next)
               {
                   ...
                   * HTTP/1.0 server then close the socket after this request. This is
                   ...
                   * problem is that if the server for some reason decides not to use
                   * persistent connections and it does not do a correct shutdown of
                   * the connection, then the response will be ReSeT. If we did
                   * pipeline then we would keep falling into this trap indefinitely.
                   * Note that for HTTP/1.0 servers, if they don't support keep-alives
