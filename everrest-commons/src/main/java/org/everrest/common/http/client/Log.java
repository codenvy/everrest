 ...
 * logging, and a value of 8 would enable authorization related logging; a value
 * of -1 would enable logging for all facilities. By default logging is
 * disabled.
 * 
...
public class Log
{
   ...
   static
   {
      ...
      TZ_OFF =
         TimeZone.getDefault().getOffset(now.get(Calendar.ERA), now.get(Calendar.YEAR), now.get(Calendar.MONTH),
            now.get(Calendar.DAY_OF_MONTH), now.get(Calendar.DAY_OF_WEEK), now.get(Calendar.MILLISECOND));
   ...
   /**
    * Write the given message to the current log if logging for the given
    * facility is enabled.
    * 
   ...
   /**
    * Write the stack trace of the given exception to the current log if logging
   ...
   private static final void writePrefix() throws IOException
   {
      ...
      int mins = secs / 60;
      int hours = mins / 60;
      logWriter.write("[" + fill2(hours) + ':' + fill2(mins - hours * 60) + ':' + fill2(secs - mins * 60) + '.'
