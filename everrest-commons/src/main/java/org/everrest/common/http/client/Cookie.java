...
public class Cookie implements Serializable
{
      ...
      cookies : while (true) // get all cookies
      {
         ...
         parts : while (true) // parse all parts
         {
            ...
            if (name.equalsIgnoreCase("expires"))
            {
                ...
                * the comma is also used between cookies themselves. To make sure we
      ...
      throws ProtocolException
   {
      ...
      else if (name.equalsIgnoreCase("domain"))
      {
          ...
          * countries use a shallow hierarchy (similar to the special TLDs defined
          * in the spec). While the rules in rfc-2965 aren't perfect either, they
          * are better. OTOH, some sites use a domain so that the host name minus
          * the domain name contains a dot (e.g. host x.x.yahoo.com and domain
          * .yahoo.com). So, for the seven special TLDs we use the 2-dot rule, and
