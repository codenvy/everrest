...
class WS2RESTAdapter implements WSMessageReceiver {
  ...
  public void onMessage(final InputMessage input) {
    ...
    if ("subscribe-channel".equalsIgnoreCase(messageType)
        || "unsubscribe-channel".equalsIgnoreCase(messageType)) {
          ...
          public void run() {
            try {
              ...
              final URI requestUri =
                  requestPath == null || requestPath.isEmpty()
                      ? URI.create("/")
                      : URI.create(
                          requestPath.charAt(0) == '/' ? requestPath : ('/' + requestPath));
              if (data != null) {
              ...
              final RestOutputMessage response = newOutputMessage(request);
              final ContainerRequest internalRequest =
                  new ContainerRequest(
                      ...
                      securityContext);
              final ContainerResponse internalResponse =
                  new ContainerResponse(new EverrestResponseWriter(response));
  ...
  /**
   * Get name of channel from input message. Expected format of message: {"channel":"my_channel"}.
   * Method return {@code null} if message is invalid.
   */
