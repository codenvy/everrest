...
public interface AuthorizationHandler {
     ...
     *
     * @param challenge
     *         the parsed challenge from the server; the host, port,
     *         scheme, realm and params are set to the values given by the server
     *         in the challenge.
     * @param req
     *         the request which provoked this response.
     * @param resp
     *         the full response.
     ...
     * info in the AuthorizationInfo class to be sent with a request. This
     * happens when either auth info is being preemptively sent or if a 401
     * response is retrieved and a matching info is found in the list of known
     * info. The intent of this method is to allow the handler to fix up the info
     * being sent based on the actual request (e.g. in digest authentication the
     * digest-uri, nonce and response-digest usually need to be recalculated).
     *
     * @param info
     *         the authorization info retrieved from the list of known info.
     * @param req
     *         the request this info is targeted for.
     * @param challenge
     *         the authorization challenge received from the server if
     ...
     * @throws IOException
     *         if an exception occurs while fixing up the info
     */
     ...
     *         the Request which provoked this response
     * @param prev
     *         the previous auth info sent, or null if none was sent
     * @param prxy
     *         the previous proxy auth info sent, or null if none was sent
     * @throws IOException
