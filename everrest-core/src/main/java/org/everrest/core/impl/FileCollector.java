...
public final class FileCollector {
  ...
  private static class FileCollectorHolder {
    private static final String name =
        String.format("%s%s", PREF, Long.toString(Math.abs(new SecureRandom().nextLong())));
    private static final FileCollector collector =
        new FileCollector(new File(System.getProperty("java.io.tmpdir"), name));
  }
