...
public class ConstructorDescriptorImpl implements ConstructorDescriptor {
  ...
  public ConstructorDescriptorImpl(
      Constructor<?> constructor, ParameterResolverFactory parameterResolverFactory) {
          ...
          if (allowedAnnotation.contains(annotationType.getName())) {
            if (parameterAnnotation != null) {
              throw new RuntimeException(
                  String.format(
                      "JAX-RS annotations on one of constructor parameters are equivocality. Annotations: %s and %s can't be applied to one parameter",
            ...
            LOG.debug(
                "Constructor parameter contains unsupported annotation: {} . It will be ignored"
                    + annotation);
          }
        }
    ...
    for (Parameter parameter : parameters) {
      ...
      if (parameterAnnotation != null) {
        ParameterResolver<?> parameterResolver =
            parameterResolverFactory.createParameterResolver(parameterAnnotation);
        try {
          ...
          if (parameterAnnotationType == PathParam.class
              || parameterAnnotationType == QueryParam.class
              || parameterAnnotationType == MatrixParam.class) {
        ...
        if (dependencies == null) {
          String errorMessage =
              String.format(
                  "Can't instantiate resource %s. DependencySupplier not found, constructor's parameter of type %s could not be injected. ",
          ...
          throw new WebApplicationException(
              Response.status(INTERNAL_SERVER_ERROR).entity(errorMessage).type(TEXT_PLAIN).build());
        }
        ...
        if (dependencyInstance == null) {
          String errorMessage =
              String.format(
                  "Can't instantiate resource %s. Constructor's parameter of type %s could not be injected. ",
                  constructor.getName(), parameter.getGenericType());
          LOG.error(errorMessage);
          if (Tracer.isTracingEnabled()) {
            Tracer.trace(errorMessage);
  ...
  public String toString() {
    ToStringHelper toStringHelper =
        MoreObjects.toStringHelper(this).add("constructor", this.constructor.getName());
