...
public class FieldInjectorImpl implements FieldInjector {
  ...
  /**
   * Setter for field. If setter available it will be used for field initialization. Otherwise field
  ...
  public FieldInjectorImpl(Field field, ParameterResolverFactory parameterResolverFactory) {
    ...
    final List<String> allowedAnnotation =
        isProvider ? PROVIDER_FIELDS_ANNOTATIONS : RESOURCE_FIELDS_ANNOTATIONS;
      ...
      if (allowedAnnotation.contains(annotationType.getName())) {
        if (annotation != null) {
          throw new RuntimeException(
              String.format(
                  "JAX-RS annotations on one of fields %s are equivocality. Annotations: %s and %s can't be applied to one field. ",
    ...
    try {
      ...
      if (annotation != null) {
        ...
        if (annotationType == PathParam.class
            ...
            || annotationType == MatrixParam.class) {
