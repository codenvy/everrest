...
public class ContainerResponse implements GenericContainerResponse {
  ...
  /**
   * Wrapper for underlying MessageBodyWriter. Need such wrapper to give possibility update HTTP
   * headers but commit them before writing the response body. NotifiesOutputStream wraps original
   * OutputStream for the HTTP body and notify OutputListener about any changes, e.g. write bytes,
   * flush or close. OutputListener processes events and initiates process of commit HTTP headers
  ...
  /**
   * Use underlying output stream as data stream. Pass all invocations to the back-end stream and
   * notify OutputListener about changes in back-end stream.
   */
  ...
  public void writeResponse() throws IOException {
    ...
    if (isNullOrWildcard(contentType)) {
      List<MediaType> acceptableWriterMediaTypes =
          context.getProviders().getAcceptableWriterMediaTypes(entity.getClass(), entityType, null);
      if (isEmptyOrContainsSingleWildcardMediaType(acceptableWriterMediaTypes)) {
        ...
        contentType =
            context.getContainerRequest().getAcceptableMediaType(acceptableWriterMediaTypes);
      }
    ...
    MessageBodyWriter entityWriter =
        context
            .getProviders()
            .getMessageBodyWriter(entity.getClass(), entityType, null, contentType);

    if (entityWriter == null) {
      String message =
          String.format("Not found writer for %s and MIME type %s", entity.getClass(), contentType);
      if (HEAD.equals(context.getContainerRequest().getMethod())) {
        LOG.warn(message);
        getHttpHeaders().putSingle(CONTENT_LENGTH, Long.toString(-1));
      } else {
        LOG.error(message);
        setResponse(Response.status(NOT_ACCEPTABLE).entity(message).type(TEXT_PLAIN).build());
        entityWriter = new StringEntityProvider();
      }
    } else {
      if (Tracer.isTracingEnabled()) {
        Tracer.trace(
            "Matched MessageBodyWriter for type %s, media type %s = (%s)",
            entity.getClass(), contentType, entityWriter);
      }

      if (getHttpHeaders().getFirst(CONTENT_LENGTH) == null) {
        long contentLength =
            entityWriter.getSize(entity, entity.getClass(), entityType, null, contentType);
        if (contentLength >= 0) {
  ...
  private boolean isEmptyOrContainsSingleWildcardMediaType(
      List<MediaType> acceptableWriterMediaTypes) {
