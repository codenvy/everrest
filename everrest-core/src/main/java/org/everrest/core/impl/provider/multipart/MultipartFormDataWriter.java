...
public class MultipartFormDataWriter {
  ...
  public void writeItems(Collection<OutputItem> items, OutputStream output, byte[] boundary)
      throws IOException {
  ...
  protected void writeItem(OutputItem item, OutputStream output, byte[] boundary)
      throws IOException {
    output.write(HYPHENS);
    output.write(boundary);
    output.write(NEW_LINE);
    ...
    final MessageBodyWriter writer =
        providers.getMessageBodyWriter(type, genericType, EMPTY, mediaType);
    if (writer == null) {
      throw new RuntimeException(
          String.format(
              "Unable to find a MessageBodyWriter for media type '%s' and class '%s'",
  ...
  private void writeHeaders(MultivaluedMap<String, String> headers, OutputStream output)
      throws IOException {
