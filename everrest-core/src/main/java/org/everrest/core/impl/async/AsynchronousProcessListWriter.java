...
public class AsynchronousProcessListWriter
    implements MessageBodyWriter<Iterable<AsynchronousProcess>> {
  ...
  @Override
  public long getSize(
      Iterable<AsynchronousProcess> asynchronousProcesses,
      Class<?> type,
      Type genericType,
  ...
  public void writeTo(
    ...
    try {
      ...
      for (AsynchronousProcess process : asynchronousProcesses) {
        writer.format(
            OUTPUT_FORMAT,
            process.getOwner() == null ? "unknown" : process.getOwner(),
            process.getId(),
            process.getStatus(),
            process.getPath());
      }
