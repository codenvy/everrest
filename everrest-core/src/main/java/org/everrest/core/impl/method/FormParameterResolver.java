 ...
 * or field annotated with &#064;FormParam annotation.
 */
public class FormParameterResolver implements ParameterResolver<FormParam> {
  ...
  private MultivaluedMap<String, String> readForm(ApplicationContext context, boolean decode)
      throws java.io.IOException {
    MediaType contentType = context.getHttpHeaders().getMediaType();
    ParameterizedType multivaluedMapType =
        newParameterizedType(MultivaluedMap.class, String.class, String.class);
    MessageBodyReader reader =
        context
            .getProviders()
            .getMessageBodyReader(MultivaluedMap.class, multivaluedMapType, null, contentType);
    if (reader == null) {
      throw new IllegalStateException(
          String.format(
              "Can't find appropriate entity reader for entity type %s and content-type %s",
    ...
    if (decode) {
      form =
          (MultivaluedMap<String, String>)
              context.getAttributes().get("org.everrest.provider.entity.decoded.form");
    } else {
      form =
          (MultivaluedMap<String, String>)
              context.getAttributes().get("org.everrest.provider.entity.encoded.form");
    }
    if (form == null) {
