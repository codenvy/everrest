...
public class HeaderHelper {
  ...
  private static final List<AcceptLanguage> ACCEPT_ALL_LANGUAGE =
      Collections.singletonList(AcceptLanguage.DEFAULT);

  /** Accept all tokens list. */
  private static final List<AcceptToken> ACCEPT_ALL_TOKENS =
      Collections.singletonList(new AcceptToken("*"));
  ...
  private static final ListHeaderProducer<AcceptMediaType> LIST_MEDIA_TYPE_PRODUCER =
      new ListHeaderProducer<>(new AcceptMediaTypeFactory());
  ...
  private static final ListHeaderProducer<AcceptLanguage> LIST_LANGUAGE_PRODUCER =
      new ListHeaderProducer<>(new AcceptLanguageFactory());
  ...
  /**
   * Accept token producer. Useful for processing 'accept-charset' and 'accept-encoding' request
   * headers.
   *
   * @see ListHeaderProducer
   */
  private static final ListHeaderProducer<AcceptToken> LIST_TOKEN_PRODUCER =
      new ListHeaderProducer<>(new AcceptTokenFactory());
   ...
   *
   * @param cookiesString the cookie string.
   ...
   *
   * @param newCookieString the new cookie string.
  ...
  public static NewCookie parseNewCookie(String newCookieString) {
    ...
    while (p < newCookieString.length()) {
      ...
      if (newCookieBuilder == null) {
        newCookieBuilder =
            aNewCookie().withName(name).withValue(value).withVersion(Cookie.DEFAULT_VERSION);
      } else {
  ...
  /**
   * Parses date header. Will try to found appropriated format for given date header. Format can be
   * one of described in <a href="http://www.w3.org/Protocols/rfc2616/rfc2616-sec3.html#sec3.3.1"
  ...
  public static Date parseDateHeader(String header) {
    ...
    throw new IllegalArgumentException(
        String.format("Not found appropriated date format for %s", header));
  }
  ...
  /**
   * Creates string representation of given object for adding to response header. Method uses {@link
   * HeaderDelegate#toString()}. If required implementation of HeaderDelegate is not accessible via
   * {@link RuntimeDelegate#createHeaderDelegate(java.lang.Class)} then method {@code toString} of
   * given object is used.
   *
   * @param o object
  ...
  public static String getHeaderAsString(Object o) {
    HeaderDelegate headerDelegate =
        RuntimeDelegate.getInstance().createHeaderDelegate(o.getClass());
   ...
   *
   * @param target string builder
   * @param appendMe string to append
   */
  ...
  /**
   * Adds quotes to {@code String} if it consists whitespaces, otherwise {@code String} will be
  ...
  /**
   * Parses quality value. Quality value must have not more then 5 characters and must not be
   * greater then 1 .
   *
  ...
  /**
   * Checks that given string is token. Token contains only US-ASCII characters except separators,
   * {@link #SEPARATORS} and controls.
   *
  ...
  /**
   * The cookies parameters can be separated by ';' or ','. This method tries to find first
   * available separator in cookie string. If both are not found then string length is returned.
   *
   ...
   *
   * <p>String \"hello \\\"someone\\\"\" will be changed to hello \"someone\"
   *
