...
public class AbstractResourceDescriptor extends BaseObjectModel implements ResourceDescriptor {
  ...
  private final TreeMap<UriPattern, Map<String, List<SubResourceMethodDescriptor>>>
      subResourceMethods;
  ...
  /**
   * Process method of resource and separate them to three types Resource Methods, Sub-Resource
  ...
  private void processMethods() {
        ...
        if (Modifier.isPublic(method.getModifiers())) {
          ...
          Annotation[] additionalAnnotations =
              securityAnnotation != null
                  ? new Annotation[] {securityAnnotation}
                  : new Annotation[0];

          if (httpMethod != null) {
            Produces producesAnnotation =
                getMethodAnnotation(method, resourceClass, Produces.class, false);
            ...
            Consumes consumesAnnotation =
                getMethodAnnotation(method, resourceClass, Consumes.class, false);
            ...
            if (subPath == null) {
              addResourceMethod(
                  method, httpMethod, methodParameters, additionalAnnotations, produces, consumes);
            } else {
    ...
    // End method processing. Start HEAD and OPTIONS resolving, see JAX-RS (JSR-311) specification
    // section 3.3.5
    resolveHeadRequest();
  ...
  private void addResourceMethod(
    ...
    ResourceMethodDescriptor resourceMethod =
        new ResourceMethodDescriptorImpl(
            method, httpMethod.value(), params, this, consumes, produces, additional);
    validateResourceMethod(resourceMethod);
    ResourceMethodDescriptor existedResourceMethod =
        findMethodResourceMediaType(
            getResourceMethods(httpMethod.value()),
            resourceMethod.consumes(),
            resourceMethod.produces());
    if (existedResourceMethod != null) {
      throw new RuntimeException(
          String.format(
              "Two resource method %s and %s with the same HTTP method, consumes and produces found",
              resourceMethod, existedResourceMethod));
    }
    resourceMethods.add(httpMethod.value(), resourceMethod);
  }

  private void addSubResourceMethod(
      Method method,
      Path subPath,
      HttpMethod httpMethod,
      List<Parameter> params,
    ...
    SubResourceMethodDescriptor subResourceMethod =
        new SubResourceMethodDescriptorImpl(
            new PathValue(subPath.value()),
            method,
            httpMethod.value(),
            params,
            this,
    ...
    Map<String, List<SubResourceMethodDescriptor>> subResourceMethods =
        getSubResourceMethods(subResourceMethod.getUriPattern());

    SubResourceMethodDescriptor existedSubResourceMethod =
        (SubResourceMethodDescriptor)
            findMethodResourceMediaType(
                subResourceMethods.get(httpMethod.value()),
                subResourceMethod.consumes(),
                subResourceMethod.produces());
    if (existedSubResourceMethod != null) {
      throw new RuntimeException(
          String.format(
              "Two sub-resource method %s and %s with the same HTTP method, path, consumes and produces found",
              subResourceMethod, existedSubResourceMethod));
    }
  ...
  private void validateResourceMethod(ResourceMethodDescriptor resourceMethod) {
    ...
    if (numberOfEntityParameters > 1) {
      throw new RuntimeException(
          String.format(
              "Method %s has %d parameters that are not annotated with JAX-RS parameter annotations, but must not have more than one",
    ...
    if (isAnyParameterAnnotatedWithFormParam && numberOfEntityParameters == 1) {
      ...
      Parameter entityParameter =
          methodParameters.stream()
              ...
              .findFirst()
              .get();
      if (entityParameter.getParameterClass() == MultivaluedMap.class
          && entityParameter.getGenericType() instanceof ParameterizedType) {
        Type[] actualTypeArguments =
            ((ParameterizedType) entityParameter.getGenericType()).getActualTypeArguments();
        if (actualTypeArguments.length == 2
            && String.class == actualTypeArguments[0]
            && String.class == actualTypeArguments[1]) {
          entityParameterIsMultivaluedMap = true;
        }
      }
      if (!entityParameterIsMultivaluedMap) {
        throw new RuntimeException(
            "At least one method's parameter is annotated with FormParam, entity parameter might not be other than MultivaluedMap<String, String>");
      }
    }
  }

  private void addSubResourceLocator(
      Method method, Path subPath, List<Parameter> params, Annotation[] additional) {
    SubResourceLocatorDescriptor resourceLocator =
        new SubResourceLocatorDescriptorImpl(
            new PathValue(subPath.value()), method, params, this, additional);
    validateSubResourceLocator(resourceLocator);
    if (subResourceLocators.containsKey(resourceLocator.getUriPattern())) {
      throw new RuntimeException(
          String.format(
              "Two sub-resource locators %s and %s with the same path found",
              resourceLocator, subResourceLocators.get(resourceLocator.getUriPattern())));
    }
  ...
  private void validateSubResourceLocator(SubResourceLocatorDescriptor resourceLocator) {
    ...
    boolean hasEntityParameter =
        methodParameters.stream().anyMatch(parameter -> parameter.getAnnotation() == null);
    if (hasEntityParameter) {
      throw new RuntimeException(
          String.format(
              "Method %s is resource locator, it must not have not JAX-RS annotated (entity) parameters",
  ...
  private Map<String, List<SubResourceMethodDescriptor>> getSubResourceMethods(
      UriPattern subResourceUriPattern) {
    Map<String, List<SubResourceMethodDescriptor>> map =
        subResourceMethods.get(subResourceUriPattern);
    if (map == null) {
  ...
  private void sortSubResourceMethods() {
    for (Map<String, List<SubResourceMethodDescriptor>> subResourceMethods :
        this.subResourceMethods.values()) {
   ...
   * @param method See {@link java.lang.reflect.Method}
  ...
  private List<Parameter> createMethodParameters(Class<?> resourceClass, Method method) {
    ...
    if (parameterClasses.length > 0) {
      ...
      boolean methodEncoded =
          getMethodAnnotation(method, resourceClass, Encoded.class, false) != null;
   ...
   *
   * <p>On receipt of a HEAD request an implementation MUST either: 1. Call method annotated with
   * request method designation for HEAD or, if none present, 2. Call method annotated with a
   * request method designation GET and discard any returned entity.
   */
  private void resolveHeadRequest() {
    ...
    if (getResources != null && getResources.size() > 0) {
      ...
      for (ResourceMethodDescriptor resourceMethod : getResources) {
        if (findMethodResourceMediaType(
                headResources, resourceMethod.consumes(), resourceMethod.produces())
            == null) {
          headResources.add(
              new ResourceMethodDescriptorImpl(
    ...
    for (Map<String, List<SubResourceMethodDescriptor>> allSubResourceMethods :
      ...
      if (getSubResources != null && getSubResources.size() > 0) {
        List<SubResourceMethodDescriptor> headSubResources =
            allSubResourceMethods.get(HttpMethod.HEAD);
        ...
        for (SubResourceMethodDescriptor subResourceMethod : getSubResources) {
          if (findMethodResourceMediaType(
                  headSubResources, subResourceMethod.consumes(), subResourceMethod.produces())
              == null) {
            headSubResources.add(
                new SubResourceMethodDescriptorImpl(
                    subResourceMethod.getPathValue(),
                    ...
                    subResourceMethod.consumes(),
                    subResourceMethod.produces(),
                    subResourceMethod.getAnnotations()));
          }
        }
      }
    }
   ...
   * According to JSR-311:
   *
   * <p>On receipt of a OPTIONS request an implementation MUST either: 1. Call method annotated with
   * request method designation for OPTIONS or, if none present, 2. Generate an automatic response
   * using the metadata provided by the JAX-RS annotations on the matching class and its methods.
   */
  private void resolveOptionsRequest() {
    ...
    if (optionResources.isEmpty()) {
      ...
      optionResources.add(
          new OptionsRequestResourceMethodDescriptorImpl(
              "OPTIONS", methodParameters, this, consumes, produces, new Annotation[0]));
    }
  }
  ...
  /**
   * Tries to get JAX-RS annotation on method from the resource class's superclasses or implemented
   * interfaces.
   *
   * @param <T> annotation type
   ...
   * @param metaAnnotation false if annotation should be on method and true in method should contain
   *     annotations that has supplied annotation
   * @return annotation from class or its ancestor or null if nothing found
   */
  private <T extends Annotation> T getMethodAnnotation(
      Method method, Class<?> resourceClass, Class<T> annotationClass, boolean metaAnnotation) {
    T annotation =
        metaAnnotation
            ? getMetaAnnotation(method, annotationClass)
      ...
      while (annotation == null && myClass != null && myClass != Object.class) {
        for (Class<?> anInterface : myClass.getInterfaces()) {
          try {
            ...
            T newAnnotation =
                metaAnnotation
                    ? getMetaAnnotation(myMethod, annotationClass)
                    : myMethod.getAnnotation(annotationClass);
            if (annotation == null) {
              annotation = newAnnotation;
            } else {
              throw new RuntimeException(
                  String.format(
                      "Conflicts of JAX-RS annotations on method %s of resource %s. "
  ...
  private <T extends Annotation> T getClassAnnotation(
      Class<?> resourceClass, Class<T> annotationClass) {
      ...
      while (annotation == null && myClass != null && myClass != Object.class) {
        for (Class<?> anInterface : myClass.getInterfaces()) {
          ...
          if (annotation == null) {
            ...
            throw new RuntimeException(
                String.format(
                    "Conflict of JAX-RS annotation on class %s. "
                        + "Class implements more that one interface and few interfaces have JAX-RS annotations.",
                    resourceClass.getName()));
          }
        }
        if (annotation == null) {
  ...
  /**
   * Check is collection of {@link org.everrest.core.resource.ResourceMethodDescriptor} already
   * contains ResourceMethodDescriptor with the same media types.
   *
   * @param resourceMethods {@link java.util.Set} of {@link
   *     org.everrest.core.resource.ResourceMethodDescriptor}
   ...
   */
  private <T extends ResourceMethodDescriptor> ResourceMethodDescriptor findMethodResourceMediaType(
      List<T> resourceMethods, List<MediaType> consumes, List<MediaType> produces) {
    if (resourceMethods == null || resourceMethods.isEmpty()) {
      return null;
    }
    ResourceMethodDescriptor matched = null;
    for (Iterator<T> iterator = resourceMethods.iterator();
        matched == null && iterator.hasNext(); ) {
  ...
  private <T extends Annotation> T getSecurityAnnotation(Method method, Class<?> clazz) {
    Class<T>[] securityAnnotationClassesClasses =
