...
public class DefaultMethodInvoker implements MethodInvoker {
  ...
  @Override
  public final Object invokeMethod(
      Object resource, GenericResourceMethod methodResource, ApplicationContext context) {
  ...
  private Object[] makeMethodParameters(
      GenericResourceMethod resourceMethod, ApplicationContext context) {
    ...
    for (Parameter methodParameter : resourceMethod.getMethodParameters()) {
      ...
      if (methodParameterAnnotation != null) {
        ParameterResolver<?> parameterResolver =
            parameterResolverFactory.createParameterResolver(methodParameterAnnotation);
        try {
          ...
          if (annotationType == MatrixParam.class
              || annotationType == QueryParam.class
              || annotationType == PathParam.class) {
            throw new WebApplicationException(
                e, Response.status(NOT_FOUND).entity(errorMsg).type(TEXT_PLAIN).build());
          }
          throw new WebApplicationException(
              e, Response.status(BAD_REQUEST).entity(errorMsg).type(TEXT_PLAIN).build());
        }
      } else {
        ...
        if (entityStream == null) {
          ...
          MessageBodyReader entityReader =
              context
                  ...
                  .getMessageBodyReader(
                      ...
                      methodParameter.getGenericType(),
          ...
          if (entityReader == null) {
            ...
            try {
              contentLength =
                  getContentLengthLong(context.getContainerRequest().getRequestHeaders());
            ...
            if (contentType == null && contentLength == 0) {
              ...
              String msg =
                  String.format(
                      "Media type %s is not supported. There is no corresponded entity reader for type %s",
                      contentType, methodParameter.getParameterClass());
              LOG.debug(msg);
              throw new WebApplicationException(
                  Response.status(UNSUPPORTED_MEDIA_TYPE).entity(msg).type(TEXT_PLAIN).build());
            }
          } else {
            try {
              if (Tracer.isTracingEnabled()) {
                Tracer.trace(
                    String.format(
                        "Matched MessageBodyReader for type %s, media type %s = (%s)",
                        methodParameter.getParameterClass(), contentType, entityReader));
              }

              MultivaluedMap<String, String> headers =
                  context.getContainerRequest().getRequestHeaders();
              params[i++] =
                  entityReader.readFrom(
                      methodParameter.getParameterClass(),
                      methodParameter.getGenericType(),
                      methodParameter.getAnnotations(),
                      contentType,
  ...
  protected void beforeInvokeMethod(
      Object resource,
      GenericResourceMethod methodResource,
      Object[] params,
      ApplicationContext context) {
    for (MethodInvokerFilter filter :
        context.getProviders().getMethodInvokerFilters(context.getPath())) {
