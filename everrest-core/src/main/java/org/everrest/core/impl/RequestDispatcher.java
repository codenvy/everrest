...
public class RequestDispatcher {
  ...
  public RequestDispatcher(ResourceBinder resourceBinder) {
    ...
    locatorDescriptorCache =
        CacheBuilder.newBuilder()
            .concurrencyLevel(8)
            .maximumSize(256)
            .expireAfterAccess(10, MINUTES)
            .build(
                new CacheLoader<Class<?>, ResourceDescriptor>() {
                  @Override
  ...
  public void dispatch(GenericContainerRequest request, GenericContainerResponse response) {
    ...
    ObjectFactory<ResourceDescriptor> resourceFactory =
        getRootResource(parameterValues, requestPath);
    if (resourceFactory == null) {
      ...
      response.setResponse(
          Response.status(NOT_FOUND)
              .entity(
                  String.format(
                      "There is no any resources matched to request path %s", requestPath))
              .type(TEXT_PLAIN)
              .build());
      return;
    }
    ...
    dispatch(
        request, response, context, resourceFactory.getObjectModel(), resource, newRequestPath);
  }
  ...
  /**
   * Get last element from path parameters. This element will be used as request path for child
   * resources.
   *
  ...
  /**
   * Process resource methods, sub-resource methods and sub-resource locators to find the best one
   * for serve request.
   *
   * @param request See {@link org.everrest.core.GenericContainerRequest}
   * @param response See {@link org.everrest.core.GenericContainerResponse}
   * @param context See {@link ApplicationContext}
   * @param resourceDescriptor the root resource descriptor or resource descriptor which was created
   *     by previous sub-resource locator
   * @param resource instance of resource class
   * @param requestPath request path, it is relative path to the base URI or other resource which
   *     was called before (one of sub-resource locators)
   */
  private void dispatch(
    ...
    Map<String, List<ResourceMethodDescriptor>> resourceMethods =
    ...
    if (resourceMethodRequested && !resourceMethods.isEmpty()) {
      ...
      boolean match =
          processResourceMethod(resourceMethods, request, response, matchedResourceMethods);
      if (match) {
        ResourceMethodDescriptor mostMatchedResourceMethod = matchedResourceMethods.get(0);
        if (Tracer.isTracingEnabled()) {
          Tracer.trace(
              "Matched resource method for method \"%s\", media type \"%s\" = (%s)",
      ...
      Map<UriPattern, SubResourceLocatorDescriptor> subResourceLocators =
          resourceDescriptor.getSubResourceLocators();

      List<SubResourceMethodDescriptor> matchedSubResourceMethods = new ArrayList<>();
      boolean match =
          processSubResourceMethod(
              subResourceMethods,
              requestPath,
              request,
              response,
              parameterValues,
              matchedSubResourceMethods);

      List<SubResourceLocatorDescriptor> matchedSubResourceLocators = new ArrayList<>();
      match |=
          processSubResourceLocator(
              subResourceLocators, requestPath, parameterValues, matchedSubResourceLocators);
      ...
      if (match) {
        ...
        if (foundMatchedSubResourceMethods && Tracer.isTracingEnabled()) {
          Tracer.trace(
              "Matched sub-resource method for method \"%s\", path \"%s\", media type \"%s\" = (%s)",
              request.getMethod(),
              requestPath,
              request.getMediaType(),
              matchedSubResourceMethods.get(0).getMethod());
        }
        if (foundMatchedSubResourceLocators && Tracer.isTracingEnabled()) {
          Tracer.trace(
              "Matched sub-resource locator for path \"%s\", media type \"%s\" = (%s)",
              requestPath, request.getMediaType(), matchedSubResourceLocators.get(0).getMethod());
        }

        if (foundMatchedSubResourceMethods
            && (!foundMatchedSubResourceLocators
                || compareSubResources(
                        matchedSubResourceMethods.get(0), matchedSubResourceLocators.get(0))
                    < 0)) {

          if (Tracer.isTracingEnabled()) {
            Tracer.trace(
                "Sub-resource method (%s) selected", matchedSubResourceMethods.get(0).getMethod());
          }

          invokeSubResourceMethod(
              requestPath, matchedSubResourceMethods.get(0), resource, context, request, response);
        } else {
          if (Tracer.isTracingEnabled()) {
            Tracer.trace(
                "Sub-resource locator (%s) selected",
                matchedSubResourceLocators.get(0).getMethod());
          }

          invokeSubResourceLocator(
              requestPath, matchedSubResourceLocators.get(0), resource, context, request, response);
        }
      } else {
        LOG.debug(
            "Not found sub-resource methods nor sub-resource locators for path {} and method {}",
            requestPath,
            request.getMethod());
      }
    }
   ...
   */
  private void invokeResourceMethod(
   ...
   */
  private void invokeSubResourceMethod(
      String requestPath,
   ...
   */
  private void invokeSubResourceLocator(
      String requestPath,
    ...
    List<LifecycleComponent> perRequestComponents =
        (List<LifecycleComponent>) context.getAttributes().get("org.everrest.lifecycle.PerRequest");
    if (perRequestComponents == null) {
      context
          .getAttributes()
          .put("org.everrest.lifecycle.PerRequest", perRequestComponents = new ArrayList<>());
    }
    // We do nothing for initialize resource since it is created by other resource but we lets to
  ...
  /**
   * Compare two sub-resources. One of it is {@link
   * org.everrest.core.resource.SubResourceMethodDescriptor} and other one id {@link
   * org.everrest.core.resource.SubResourceLocatorDescriptor}. First compare UriPattern, see {@link
   * org.everrest.core.uri.UriPattern#URIPATTERN_COMPARATOR}. NOTE URI comparator compare
   * UriPatterns for descending sorting. So it it return negative integer then it minds
   * SubResourceMethodDescriptor has higher priority by UriPattern comparison. If comparator return
   * positive integer then SubResourceLocatorDescriptor has higher priority. And finally if zero was
   * returned then UriPattern is equals, in this case SubResourceMethodDescriptor must be selected.
   *
   * @param subResourceMethod See {@link org.everrest.core.resource.SubResourceMethodDescriptor}
   * @param subResourceLocator See {@link org.everrest.core.resource.SubResourceLocatorDescriptor}
   * @return result of comparison sub-resources
   */
  private int compareSubResources(
      SubResourceMethodDescriptor subResourceMethod,
      SubResourceLocatorDescriptor subResourceLocator) {
    int result =
        UriPattern.URIPATTERN_COMPARATOR.compare(
            subResourceMethod.getUriPattern(), subResourceLocator.getUriPattern());
  ...
  private void processResponse(
    ...
    if (methodInvocationResult == null
        || methodInvocationResult.getClass() == void.class
        || methodInvocationResult.getClass() == Void.class) {
      ...
      final String externalJobUri =
          context.getBaseUriBuilder().path(internalJobUri).build().toString();
      response.setResponse(
      ...
      if (Response.class.isAssignableFrom(methodInvocationResult.getClass())) {
        ...
        if (resultResponse.getMetadata().getFirst(CONTENT_TYPE) == null
            && resultResponse.getEntity() != null) {
   ...
   */
  private <T extends ResourceMethodDescriptor> boolean processResourceMethod(
      Map<String, List<T>> resourceMethods,
      GenericContainerRequest request,
      GenericContainerResponse response,
    ...
    if (resourceMethodCandidates.isEmpty()) {
      response.setResponse(
          Response.status(NOT_ACCEPTABLE).entity("Not Acceptable").type(TEXT_PLAIN).build());
      return false;
    }
  ...
  private <T extends ResourceMethodDescriptor> Comparator<T> byAcceptMediaTypeComparator(
      List<AcceptMediaType> acceptMediaTypes) {
    return (resourceMethodOne, resourceMethodTwo) ->
        Float.compare(
            findFistCompatibleAcceptMediaType(acceptMediaTypes, resourceMethodTwo.produces())
                .getQvalue(),
            findFistCompatibleAcceptMediaType(acceptMediaTypes, resourceMethodOne.produces())
                .getQvalue());
  }

  private <T extends ResourceMethodDescriptor> Predicate<T> notAcceptableFilter(
      List<AcceptMediaType> acceptMediaTypes) {
    return resourceMethod ->
        findFistCompatibleAcceptMediaType(acceptMediaTypes, resourceMethod.produces()) != null;
  }

  private <T extends ResourceMethodDescriptor> Collection<String> getAllow(
      Map<String, List<T>> resourceMethods) {
   ...
   */
  private boolean processSubResourceMethod(
      Map<UriPattern, Map<String, List<SubResourceMethodDescriptor>>> subResourceMethods,
    ...
    for (Entry<UriPattern, Map<String, List<SubResourceMethodDescriptor>>> entry :
        subResourceMethods.entrySet()) {
    ...
    if (resourceMethods == null) {
      response.setResponse(
          Response.status(NOT_FOUND)
              .entity(
                  String.format(
                      "There is no any resources matched to request path %s", requestedPath))
              .type(TEXT_PLAIN)
              .build());
      return false;
    }
   ...
   */
  private boolean processSubResourceLocator(
      Map<UriPattern, SubResourceLocatorDescriptor> subResourceLocators,
      String requestedPath,
   ...
   */
  private ObjectFactory<ResourceDescriptor> getRootResource(
      List<String> parameterValues, String requestPath) {
    ObjectFactory<ResourceDescriptor> resourceFactory =
    ...
    if (resourceFactory != null) {
      if (Tracer.isTracingEnabled()) {
        ...
        Tracer.trace(
            "Matched root resource for request path \"%s\" = (@Path \"%s\", %s)",
            requestPath,
            resourceDescriptor.getPathValue().getPath(),
            resourceDescriptor.getObjectClass());
      }
    }
