...
public class JsonParser {
      ...
      switch (c = next()) {
        case END_OF_STREAM:
          throw new JsonException(
              "Syntax error. Unexpected end of object. Object must end by '}'.");
        case '}':
          endObject();
          return;
        case '{':
  ...
  private void readKey() throws JsonException {
    ...
    if (c != '"') {
      throw new JsonException(
          String.format("Syntax error. Key must start from quote, but found '%s'.", c));
    }
    pushBack(c);
  ...
  /**
   * Get next char from stream, skipping whitespace and comments. Comments: One line comment from //
   * to end of line; Multi-line comments from &#8260;* to *&#8260;
   *
   ...
   *
   * @param expectedCharacter the expected char.
   * @return the next char.
   * @throws JsonException if JSON document has wrong format or i/o error occurs.
   */
  private char assertNextIs(char expectedCharacter) throws JsonException {
    ...
    throw new JsonException(
        String.format("Expected for '%s' but found '%s'.", expectedCharacter, c));
  }

  /**
   * Get next char from stream. And check is this char presents in given string.
   *
   * @param expectedCharacters the string.
   * @return the next char.
   * @throws JsonException if JSON document has wrong format or i/o error occurs.
   */
