...
public class ContainerRequest implements GenericContainerRequest {
  ...
  public Variant selectVariant(List<Variant> variants) {
    checkArgument(
        !(variants == null || variants.isEmpty()), "The list of variants is null or empty");
    return variantsHandler.handleVariants(this, variants);
  }

  /**
   * If accept-language header does not present or its length is null then default language list
   * will be returned. This list contains only one element Locale with language '*', and it minds
    ...
    if (contentLanguage == null && httpHeaders.getFirst(CONTENT_LANGUAGE) != null) {
      contentLanguage =
          RuntimeDelegate.getInstance()
              .createHeaderDelegate(Locale.class)
              .fromString(httpHeaders.getFirst(CONTENT_LANGUAGE));
    }
   ...
   *
   * @param etag the ETag
   * @return ResponseBuilder with status 412 (precondition failed) if If-Match header does NOT MATCH
   *     to ETag or null otherwise
   */
   ...
   *
   * @param etag the ETag
   * @return ResponseBuilder with status 412 (precondition failed) if If-None-Match header is MATCH
   *     to ETag and HTTP method is not GET or HEAD. If method is GET or HEAD and If-None-Match is
   *     MATCH to ETag then ResponseBuilder with status 304 (not modified) will be returned.
   */
   ...
   * @return ResponseBuilder with status 412 (precondition failed) if lastModified time is greater
   *     then unmodifiedSince otherwise return null. If date format in header If-Unmodified-Since is
   *     wrong also null returned
   */
   ...
   *
   * @param lastModified the last modified time
   * @return ResponseBuilder with status 304 (not modified) if lastModified time is greater then
   *     modifiedSince otherwise return null. If date format in header If-Modified-Since is wrong
   *     also null returned
   */
