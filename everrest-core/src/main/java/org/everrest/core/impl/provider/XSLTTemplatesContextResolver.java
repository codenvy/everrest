...
public class XSLTTemplatesContextResolver implements ContextResolver<XSLTTemplatesContextResolver> {
  ...
  private final ConcurrentMap<String, Templates> templatesMap =
   ...
   * @throws IllegalArgumentException if templates with specified {@code name} already registered
   * @throws IOException if any i/o errors occurs
   * @throws SAXException if given source can not be parsed
   * @throws TransformerConfigurationException if templates handler can't be initialized
   * @see Templates
   * @see TransformerConfigurationException
   */
  public void addAsTemplate(String name, Source source)
  ...
  public void addTemplates(String name, Templates templates) {
    if (templatesMap.putIfAbsent(name, templates) != null) {
      throw new IllegalArgumentException(
          String.format("Template with name '%s' already registered", name));
    }
  }
