...
public class HeaderParameterParser {
   ...
   *
   * @param header header header string
   * @return header parameter
   * @throws ParseException if string can't be parsed or contains illegal characters
   */
  ...
  /**
   * Reads token from header string, token is not quoted string and does not contains any
   * separators. See <a href="http://www.w3.org/Protocols/rfc2616/rfc2616-sec2.html">HTTP1.1
   * specification</a>.
   *
    ...
    if (token != null) {
      ...
      if ((err = HeaderHelper.isToken(token)) != -1) {
        throw new ParseException(
            String.format("Token '%s' contains not legal characters at %d", token, err), err);
      }
    }
