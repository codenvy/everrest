...
public class UriBuilderImpl extends UriBuilder {
    ...
    private String createUriWithValues(Object[] values, boolean encode, boolean encodeSlashInPath) {
        return createUriWithValues(schema, authority, userInfo, host, port, path.toString(), query.toString(), fragment, values, encode,
                                   encodeSlashInPath, false);
    }

    private String createUriWithValues(Map<String, ?> values, boolean encode, boolean encodeSlashInPath) {
        return createUriWithValues(schema, authority, userInfo, host, port, path.toString(), query.toString(), fragment, values, encode,
                                   encodeSlashInPath, false);
    }
    ...
    public String toTemplate() {
        return createUriWithValues(schema,
                                   authority,
                                   userInfo,
                                   host,
                                   port,
    ...
    private void preparePath() {
        ...
        if (path.length() == 0 && !isNullOrEmpty(host) && (query.length() > 0 || !isNullOrEmpty(fragment))) {
            path.append('/');
        }
    }
    ...
    public UriBuilder path(Class resource, String method) {
        ...
        for (int i = 0, length = methods.length; i < length; i++) {
            ...
            if (m.getName().equals(method)) {
                if (matched != null && !(matched.isSynthetic() || m.isSynthetic())) {
                    throw new IllegalArgumentException(String.format("More then one method with name %s found", method));
                } else {
                    matched = m;
                }
            }
        ...
        if (matched == null) {
            throw new IllegalArgumentException(String.format("Method %s not found at resource class %s", method, resource.getName()));
        }
        ...
        if (query.length() > 0) {
            ...
            while (p < queryStringLength) {
                ...
                if (n > p) {
                    ...
                    String pairName = eq == pair.length()
                                      ? pair : pair.substring(0, eq);
    ...
    private void validateQueryString(String queryString) {
        ...
        while (p < queryString.length()) {
            if (charAtIs(queryString, p, '=')) {
                ...
                throw new IllegalArgumentException(String.format("Invalid query string at %d. Query parameter name is empty.", p));
            }
     ...
     */
    private void appendUriPart(StringBuilder sb, String str, int component, Map<String, ?> values, boolean encode, boolean asTemplate) {
        int p = 0;
        int n = 0;
        ...
        while (p < length) {
            ...
            if (charAtIs(str, p, '{')) {
                ...
                if (charAtIs(str, n, '}')) {
                    ...
                    if (paramValue == null) {
                        if (!asTemplate) {
                            throw new IllegalArgumentException(String.format("Not found corresponding value for parameter %s", paramName));
                        }
     ...
     * @return offset
     */
    private int appendUriPart(StringBuilder sb, String str, int component, Object[] sourceValues, int offset, Map<String, String> values,
                              boolean encode, boolean asTemplate) {
        int p = 0;
        int n = 0;
        int length = str.length();
        while (p < length) {
     ...
     */
    private String createUriWithValues(String schema,
                                       String authority,
                                       String userInfo,
                                       String host,
                                       String port,
    ...
    private String createUriWithValues(String schema,
        ...
        if (!isNullOrEmpty(path)) {
            ...
            p = appendUriPart(sb, path, encodeSlashInPath ? PATH_SEGMENT : PATH, values, p, m, encode, asTemplate);
        }
