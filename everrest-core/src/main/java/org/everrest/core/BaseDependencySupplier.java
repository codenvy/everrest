...
public abstract class BaseDependencySupplier implements DependencySupplier {
  ...
  public BaseDependencySupplier(
      Class<? extends Annotation> injectAnnotationClass, DependencyNameDetector nameDetector) {
  ...
  /**
   * Get instance of dependency by name. This is optional capability so by default this method
   * returns {@code null}. Override it if back-end (e.g. IoC container) supports getting components
   * by key (name).
   *
   * @param name of dependency
   * @return object of required type or null if instance described by {@code name} may not be
   *     produced
   * @throws RuntimeException if any error occurs while creating instance of {@code name}
   */
  ...
  public javax.inject.Provider<?> getProvider(Type providerType) {
    ...
    if (((ParameterizedType) providerType).getRawType() != javax.inject.Provider.class) {
      throw new RuntimeException(
          String.format("Type %s is not javax.inject.Provider. ", providerType));
    }
  ...
  /**
   * Implementation of this interface determine name of dependency required for constructors or
   * fields of Resource or Provider. Typically the name of the dependency may be set by annotation
   * with value as component's qualifier, e.g. &#064javax.inject.Named.
   */
