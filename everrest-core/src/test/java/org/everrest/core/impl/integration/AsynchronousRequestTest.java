...
public class AsynchronousRequestTest extends BaseTest {
  ...
  public void providesListOfAsynchronousJobsAsJson() throws Exception {
    ...
    Collection<AsynchronousProcess> processes =
        (Collection<AsynchronousProcess>) response.getEntity();
  ...
  private String startAsynchronousJobAndGetItsUrl(
      String url,
      String method,
  ...
  private ContainerResponse getAsynchronousResponse(
      String jobUrl, ByteArrayContainerResponseWriter writer) throws Exception {
    ContainerResponse response;
    final long endTime = System.currentTimeMillis() + 5000;
    synchronized (this) {
      while ((response = launcher.service("GET", jobUrl, "", null, null, writer, null)).getStatus()
              == 202
