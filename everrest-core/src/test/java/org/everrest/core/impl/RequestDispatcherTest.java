...
public class RequestDispatcherTest {
  ...
  private void mockApplicationContext() {
    ...
    when(applicationContext.getMethodInvoker(isA(GenericResourceMethod.class)))
    ...
    when(applicationContext.getAttributes().get("org.everrest.lifecycle.PerRequest"))
        .thenReturn(newArrayList());
    when(applicationContext.getBaseUriBuilder())
        .thenReturn(UriBuilderImpl.fromPath("http://localhost:8080/servlet"));
  ...
  public void callsResourceMethod() throws Exception {
    ...
    ResourceMethodDescriptor resourceMethod =
        mockResourceMethod(
            Resource.class.getMethod("echo", String.class),
            "POST",
            newArrayList(WILDCARD_TYPE),
            newArrayList(WILDCARD_TYPE));
    ObjectFactory resourceFactory =
        mockResourceFactory(resource, newArrayList(resourceMethod), newArrayList(), newArrayList());

    matchRequestPath();
    when(resources.getMatchedResource(eq("/a/b"), anyList())).thenReturn(resourceFactory);

    when(methodInvoker.invokeMethod(same(resource), same(resourceMethod), same(applicationContext)))
        .thenReturn("foo");
  ...
  @Test
  public void returnsResponseWithStatus_NO_CONTENT_WhenResourceMethodReturnsNull()
      throws Exception {
    when(applicationContext.getPathSegments(false)).thenReturn(createPathSegments("a", "b;x=y"));
  ...
  @Test
  public void returnsResponseWithStatus_ACCEPTED_WhenResourceMethodReturnsAsynchronousJob()
      throws Exception {
    when(applicationContext.getPathSegments(false)).thenReturn(createPathSegments("a", "b;x=y"));
    ...
    assertEquals(
        "http://localhost:8080/servlet/async/1",
        argumentCaptor.getValue().getHeaderString(LOCATION));
  }
  ...
  public void callsResourceMethodRespectsConsumedMediaTypes() throws Exception {
    ...
    Resource resource = new Resource();
    ResourceMethodDescriptor resourceMethodOne =
        mockResourceMethod(
            Resource.class.getMethod("echo", String.class),
            "POST",
            newArrayList(TEXT_XML_TYPE, APPLICATION_XML_TYPE),
            newArrayList(WILDCARD_TYPE));
    ResourceMethodDescriptor resourceMethodTwo =
        mockResourceMethod(
            Resource.class.getMethod("echo", String.class),
            "POST",
    ...
    ObjectFactory resourceFactory =
        mockResourceFactory(
            resource,
            newArrayList(resourceMethodOne, resourceMethodTwo),
            newArrayList(),
            newArrayList());
    ...
    when(methodInvoker.invokeMethod(
            same(resource), same(resourceMethodOne), same(applicationContext)))
        .thenReturn("foo");
    when(methodInvoker.invokeMethod(
            same(resource), same(resourceMethodTwo), same(applicationContext)))
        .thenReturn("bar");
  ...
  public void returnsResponseWithStatus_NOT_FOUND_WhenResourceMethodNotFound() throws Exception {
    ...
    Resource resource = new Resource();
    ObjectFactory resourceFactory =
        mockResourceFactory(resource, newArrayList(), newArrayList(), newArrayList());
  ...
  @Test
  public void
      returnsResponseWithStatus_METHOD_NOT_ALLOWED_WhenNotFoundResourceMethodWithRequestedHttpMethod()
          throws Exception {
    when(applicationContext.getPathSegments(false)).thenReturn(createPathSegments("a", "b;x=y"));
  ...
  @Test
  public void
      returnsResponseWithStatus_UNSUPPORTED_MEDIA_TYPE_WhenResourceMethodDoesNotSupportContentTypeOfRequest()
          throws Exception {
    when(applicationContext.getPathSegments(false)).thenReturn(createPathSegments("a", "b;x=y"));
  ...
  public void callsResourceMethodRespectsProducedMediaTypes() throws Exception {
    when(applicationContext.getPathSegments(false)).thenReturn(createPathSegments("a", "b;x=y"));
    when(request.getAcceptMediaTypeList())
        .thenReturn(
            newArrayList(
                new AcceptMediaType(TEXT_XML_TYPE, 0.7f), new AcceptMediaType(TEXT_PLAIN_TYPE)));
  ...
  @Test
  public void
      returnsResponseWithStatus_NOT_ACCEPTABLE_WhenResourceMethodDoesNotProduceContentAcceptableByCaller()
          throws Exception {
    when(applicationContext.getPathSegments(false)).thenReturn(createPathSegments("a", "b;x=y"));
    when(request.getAcceptMediaTypeList())
        .thenReturn(newArrayList(new AcceptMediaType(TEXT_PLAIN_TYPE)));
  ...
  public void callsSubResourceMethod() throws Exception {
    when(applicationContext.getPathSegments(false))
        .thenReturn(createPathSegments("a", "b", "c", "d;x=y"));

    Resource resource = new Resource();
    SubResourceMethodDescriptor subResourceMethod =
        mockSubResourceMethod(
            Resource.class.getMethod("echo", String.class),
            "c/d",
            "POST",
            newArrayList(WILDCARD_TYPE),
    ...
    ObjectFactory resourceFactory =
        mockResourceFactory(
            resource, newArrayList(), newArrayList(subResourceMethod), newArrayList());

    matchRequestPath("c/d");

    when(resources.getMatchedResource(eq("/a/b/c/d"), anyList())).thenReturn(resourceFactory);
    when(methodInvoker.invokeMethod(
            same(resource), same(subResourceMethod), same(applicationContext)))
        .thenReturn("foo");
  ...
  public void callsSubResourceMethodRespectsProducedMediaTypes() throws Exception {
    ...
    ObjectFactory resourceFactory =
        mockResourceFactory(
            resource,
            newArrayList(),
            newArrayList(subResourceMethodOne, subResourceMethodTwo),
            newArrayList());
    ...
    when(methodInvoker.invokeMethod(
            same(resource), same(subResourceMethodOne), same(applicationContext)))
        .thenReturn("foo");
    when(methodInvoker.invokeMethod(
            same(resource), same(subResourceMethodTwo), same(applicationContext)))
        .thenReturn("<bar/>");
  ...
  @Test
  public void findsSubResourceWithRequestedHttpMethodWhenPathMatchesToMoreThanOneSubResourcesPaths()
      throws Exception {
  ...
  @Test
  public void
      returnsResponseWithStatus_METHOD_NOT_ALLOWED_WhenNotFoundSubResourceMethodWithRequestedHttpMethod()
          throws Exception {
    when(applicationContext.getPathSegments(false))
        .thenReturn(createPathSegments("a", "b", "c", "d;x=y"));
  ...
  @Test
  public void
      returnsResponseWithStatus_UNSUPPORTED_MEDIA_TYPE_WhenSubResourceMethodDoesNotSupportContentTypeOfRequest()
          throws Exception {
    when(applicationContext.getPathSegments(false))
        .thenReturn(createPathSegments("a", "b", "c", "d;x=y"));
    when(request.getMediaType()).thenReturn(APPLICATION_JSON_TYPE);
  ...
  @Test
  public void
      returnsResponseWithStatus_NOT_ACCEPTABLE_WhenSubResourceMethodDoesNotProduceContentAcceptableByCaller()
          throws Exception {
    when(applicationContext.getPathSegments(false))
        .thenReturn(createPathSegments("a", "b", "c", "d;x=y"));
    when(request.getAcceptMediaTypeList())
  ...
  public void callsSubResourceLocator() throws Exception {
    ...
    ObjectFactory resourceFactory =
        mockResourceFactory(
            locatorResource, newArrayList(), newArrayList(), newArrayList(subResourceLocator));
    ...
    when(methodInvoker.invokeMethod(
            same(locatorResource), same(subResourceLocator), same(applicationContext)))
        .thenReturn(subResource);
    when(methodInvoker.invokeMethod(
            same(subResource), isA(ResourceMethodDescriptor.class), same(applicationContext)))
        .thenReturn("foo");
  ...
  @Test
  public void
      returnsResponseWithStatus_METHOD_NOT_ALLOWED_WhenSubResourceLocatorProducesResourceThatDoesNotSupportRequestedHttpMethod()
          throws Exception {
  ...
  @Test
  public void
      returnsResponseWithStatus_UNSUPPORTED_MEDIA_TYPE_WhenSubResourceLocatorProducesResourceThatDoesNotSupportContentTypeOfRequest()
          throws Exception {
    when(request.getMediaType()).thenReturn(APPLICATION_JSON_TYPE);
    when(applicationContext.getPathSegments(false))
        .thenReturn(createPathSegments("a", "b", "c", "d;x=y"));
  ...
  @Test
  public void
      returnsResponseWithStatus_NOT_ACCEPTABLE_WhenSubResourceLocatorProducesResourceThatDoesNotProduceContentAcceptableByCaller()
          throws Exception {
    when(applicationContext.getPathSegments(false))
        .thenReturn(createPathSegments("a", "b", "c", "d;x=y"));
    when(request.getAcceptMediaTypeList())
  ...
  @Test
  public void subResourceMethodTakesPrecedenceOverSubResourceLocatorWithTheSamePath()
      throws Exception {
    when(applicationContext.getPathSegments(false))
        .thenReturn(createPathSegments("a", "b", "c", "d;x=y"));
    ...
    SubResourceMethodDescriptor subResourceMethod =
        mockSubResourceMethod(
            LocatorResource.class.getMethod("echo", String.class),
            "c/d",
            "POST",
            newArrayList(WILDCARD_TYPE),
            newArrayList(WILDCARD_TYPE));
    ...
    SubResource subResource = new SubResource();
    when(methodInvoker.invokeMethod(
            same(locatorResource), same(subResourceMethod), same(applicationContext)))
        .thenReturn("foo");
    when(methodInvoker.invokeMethod(
            same(locatorResource), same(subResourceLocator), same(applicationContext)))
        .thenReturn(subResource);
  ...
  private ObjectFactory mockResourceFactory(
      Object resource,
      List<ResourceMethodDescriptor> resourceMethods,
      List<SubResourceMethodDescriptor> subResourceMethods,
      List<SubResourceLocatorDescriptor> subResourceLocators)
      throws Exception {
    ResourceDescriptor resourceDescriptor = mock(ResourceDescriptor.class, RETURNS_DEEP_STUBS);

    Map<String, List<ResourceMethodDescriptor>> resourceMethodsMap =
        createResourceMethodsMap(resourceMethods);
    when(resourceDescriptor.getResourceMethods()).thenReturn(resourceMethodsMap);

    Map<UriPattern, Map<String, List<SubResourceMethodDescriptor>>> subResourceMethodsMap =
        createSubResourceMethodsMap(subResourceMethods);
    when(resourceDescriptor.getSubResourceMethods()).thenReturn(subResourceMethodsMap);

    Map<UriPattern, SubResourceLocatorDescriptor> subResourceLocatorsMap =
        createSubResourceLocatorsMap(subResourceLocators);
  ...
  private Map<String, List<ResourceMethodDescriptor>> createResourceMethodsMap(
      List<ResourceMethodDescriptor> resourceMethods) {
    Map<String, List<ResourceMethodDescriptor>> resourceMethodsMap =
        resourceMethods.stream().collect(groupingBy(ResourceMethodDescriptor::getHttpMethod));

    ResourceMethodComparator resourceMethodComparator = new ResourceMethodComparator();

    resourceMethodsMap
        .values()
        .forEach(_resourceMethods -> Collections.sort(_resourceMethods, resourceMethodComparator));
    return resourceMethodsMap;
  }

  private Map<UriPattern, Map<String, List<SubResourceMethodDescriptor>>>
      createSubResourceMethodsMap(List<SubResourceMethodDescriptor> subResourceMethods) {
    Map<UriPattern, Map<String, List<SubResourceMethodDescriptor>>> subResourceMethodsMap =
        new TreeMap<>(new UriPatternComparator());
    for (SubResourceMethodDescriptor subResourceMethod : subResourceMethods) {
      Map<String, List<SubResourceMethodDescriptor>> resourceMethodsMap =
          subResourceMethodsMap.get(subResourceMethod.getUriPattern());
      if (resourceMethodsMap == null) {
        subResourceMethodsMap.put(
            subResourceMethod.getUriPattern(), resourceMethodsMap = new HashMap<>());
      }
      List<SubResourceMethodDescriptor> resourceMethods =
          resourceMethodsMap.get(subResourceMethod.getHttpMethod());
      if (resourceMethods == null) {
        resourceMethodsMap.put(
            subResourceMethod.getHttpMethod(), resourceMethods = new ArrayList<>());
      }
  ...
  private Map<UriPattern, SubResourceLocatorDescriptor> createSubResourceLocatorsMap(
      List<SubResourceLocatorDescriptor> subResourceLocators) {
    Map<UriPattern, SubResourceLocatorDescriptor> subResourceLocatorsMap =
        new TreeMap<>(new UriPatternComparator());
  ...
  private ResourceMethodDescriptor mockResourceMethod(
      Method method, String httpMethod, List<MediaType> consumes, List<MediaType> produces) {
  ...
  private SubResourceMethodDescriptor mockSubResourceMethod(
      Method method,
      String path,
      String httpMethod,
  ...
  private UriPattern mockUriPattern(String path) {
    ...
    when(uriPattern.match(anyString(), anyList()))
        .thenAnswer(
            invocation -> {
