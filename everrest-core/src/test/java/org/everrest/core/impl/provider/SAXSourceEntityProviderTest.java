...
public class SAXSourceEntityProviderTest {
  ...
  private static final Map<String, String> XPATH_EXPR_TO_VALUE =
      ImmutableMap.of(
          "/book/@send-by-post", "true",
  ...
  public void isReadableForSAXSource() throws Exception {
    assertTrue(
        saxSourceEntityProvider.isReadable(SAXSource.class, null, null, APPLICATION_XML_TYPE));
  }

  @Test
  public void isNotReadableForTypeOtherThanSAXSource() throws Exception {
    assertFalse(
        saxSourceEntityProvider.isReadable(DOMSource.class, null, null, APPLICATION_XML_TYPE));
  }

  @Test
  public void isWritableForSAXSource() throws Exception {
    assertTrue(
        saxSourceEntityProvider.isWriteable(SAXSource.class, null, null, APPLICATION_XML_TYPE));
  }

  @Test
  public void isNotWritableForTypeOtherThanSAXSource() throws Exception {
    assertFalse(
        saxSourceEntityProvider.isWriteable(DOMSource.class, null, null, APPLICATION_XML_TYPE));
  }

  @Test
  public void readsContentOfEntityStreamAsSAXSource() throws Exception {
    SAXSource saxSource =
        saxSourceEntityProvider.readFrom(
            SAXSource.class,
            null,
            null,
    ...
    Document document =
        DocumentBuilderFactory.newInstance().newDocumentBuilder().parse(saxSource.getInputSource());
  ...
  public void writesSAXSourceToOutputStream() throws Exception {
    ...
    saxSourceEntityProvider.writeTo(
        new SAXSource(new InputSource(new ByteArrayInputStream(TEST_XML_CONTENT_BYTES))),
        SAXSource.class,
        null,
        null,
        APPLICATION_XML_TYPE,
  ...
  private void assertThatXmlDocumentContainsAllExpectedNodes(Document document)
    ...
    for (Map.Entry<String, String> entry : XPATH_EXPR_TO_VALUE.entrySet()) {
      ...
      assertEquals(
          String.format("Unexpected result in XML Document for expression: %s", xpathExpression),
          expectedResult,
          result);
    }
  }
