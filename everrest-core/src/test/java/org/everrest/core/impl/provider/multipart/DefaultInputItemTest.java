...
public class DefaultInputItemTest {
    ...
    public void getsBodyAsCustomType() throws Exception {
        ...
        when(reader.readFrom(eq(type), eq(genericType), aryEq(annotations), eq(TEXT_PLAIN_TYPE), any(), eq(in))).thenReturn(bodyAsList);
        when(providers.getMessageBodyReader(eq(type), eq(genericType), aryEq(annotations), eq(TEXT_PLAIN_TYPE))).thenReturn(reader);

        assertEquals(bodyAsList, inputItem.getBody(type, genericType));
    }

    @Test
    public void failsGetBodyAsCustomTypeWhenMessageBodyReaderForThisTypeIsNotAvailable() throws Exception {
        Class<List> type = List.class;
        ParameterizedType genericType = newParameterizedType(List.class, String.class);
        InputStream in = mock(InputStream.class);
