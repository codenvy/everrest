...
public class FormParameterResolverTest {
  ...
  public void resolvesDecodedFormParamAsString() throws Exception {
    ...
    when(typeProducer.createValue("foo", decodedForm, null))
        .thenReturn(decodedForm.getFirst("foo"));
  ...
  public void resolvesEncodedFormParamAsString() throws Exception {
    ...
    when(typeProducer.createValue("foo", encodedForm, null))
        .thenReturn(encodedForm.getFirst("foo"));
  ...
  @Test
  public void throwsIllegalStateExceptionWhenFormMessageBodyReaderIsNotAvailable()
      throws Exception {
    ...
    when(applicationContext
            ...
            .getMessageBodyReader(
                eq(type), eq(genericType), any(), eq(APPLICATION_FORM_URLENCODED_TYPE)))
        .thenReturn(null);
  ...
  private ApplicationContext mockApplicationContext() {
    ...
    when(applicationContext.getHttpHeaders().getMediaType())
        .thenReturn(APPLICATION_FORM_URLENCODED_TYPE);
    when(applicationContext.getAttributes().get("org.everrest.provider.entity.decoded.form"))
        .thenReturn(decodedForm);
    when(applicationContext.getAttributes().get("org.everrest.provider.entity.encoded.form"))
        .thenReturn(encodedForm);
  ...
  private void assertThatMessageBodyWriterWasCalled() throws Exception {
    verify(messageBodyReader)
        .readFrom(
            eq(MultivaluedMap.class),
