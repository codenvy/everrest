...
public class DefaultMethodInvokerTest {
  ...
  public void notifiesMethodInvokerFiltersBeforeMethodInvocation() throws Exception {
    ...
    when(applicationContext.getProviders().getMethodInvokerFilters(RESOURCE_PATH))
        .thenReturn(newArrayList(methodInvokerFilter));
  ...
  @Test
  public void invokesMethodWithNullEntityParameterWhenEntityStreamDoesNotPresent()
      throws Exception {
  ...
  @Test
  public void throwsWebApplicationExceptionWhenMessageBodyReaderForEntityParameterIsNotAvailable()
      throws Exception {
    when(applicationContext
            .getProviders()
            .getMessageBodyReader(
                (Class) entityParameter.getParameterClass(),
  ...
  @Test
  public void
      invokesMethodWithNullEntityParameterWhenMessageBodyReaderIsNotAvailableAndBothContentTypeAndContentLengthAreNotSet()
          throws Exception {
  ...
  public void rethrowWebApplicationExceptionThatOccursWhenReadEntityParameter() throws Exception {
    ...
    when(messageBodyReader.readFrom(
            (Class) entityParameter.getParameterClass(),
            entityParameter.getGenericType(),
            entityParameter.getAnnotations(),
            applicationContext.getContainerRequest().getMediaType(),
            applicationContext.getContainerRequest().getRequestHeaders(),
            applicationContext.getContainerRequest().getEntityStream()))
        .thenThrow(thrownByMessageBodyReader);
  ...
  @Test
  public void wrapsExceptionThatOccursWhenReadEntityParameterWithInternalException()
      throws Exception {
  ...
  public static Object[][]
      annotationTypesWhenNeedToThrowWebApplicationExceptionWithStatusNotFound() {
    return new Object[][] {{MatrixParam.class}, {QueryParam.class}, {PathParam.class}};
  }

  @UseDataProvider("annotationTypesWhenNeedToThrowWebApplicationExceptionWithStatusNotFound")
  @Test
  public <A extends Annotation>
      void throwsWebApplicationExceptionWithStatusNotFoundWhenFailResolveParameterAnnotatedWith(
    ...
    when(parameterResolver.resolve(annotatedParameter, applicationContext))
        .thenThrow(thrownByParameterResolver);
  ...
  @DataProvider
  public static Object[][]
      annotationTypesWhenNeedToThrowWebApplicationExceptionWithStatusBadRequest() {
    return new Object[][] {
      {CookieParam.class}, {Context.class}, {FormParam.class}, {HeaderParam.class}, {Property.class}
    };
  }
  ...
  @Test
  public <A extends Annotation>
      void throwsWebApplicationExceptionWithStatusBadRequestWhenFailResolveParameterAnnotatedWith(
          Class<A> annotationType) throws Exception {
    when(resourceMethod.getMethodParameters()).thenReturn(newArrayList(annotatedParameter));
    when(parameterAnnotation.annotationType()).thenReturn((Class) annotationType);
    Exception thrownByParameterResolver = new Exception();
  ...
  public void rethrowWebApplicationExceptionThatThrownByInvocatedMethod() throws Exception {
    ...
    methodInvoker.invokeMethod(
        new EchoResource(thrownByMethod), resourceMethod, applicationContext);
  }
  ...
  private void mockParameterResolverFactory() throws Exception {
    ...
    when(parameterResolver.resolve(annotatedParameter, applicationContext))
        .thenReturn(ARGUMENT_VALUE);

    parameterResolverFactory = mock(ParameterResolverFactory.class);
    when(parameterResolverFactory.createParameterResolver(parameterAnnotation))
        .thenReturn(parameterResolver);
  }
  ...
  private void mockApplicationContext() {
    ...
    when(applicationContext.getProviders().getMethodInvokerFilters(anyString()))
        .thenReturn(newArrayList());
    when(applicationContext.getQueryParameters()).thenReturn(new MultivaluedHashMap<>());
    when(applicationContext.getContainerRequest().getRequestHeaders())
