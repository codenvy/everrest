...
public class DependencySupplierImplTest {
  ...
  public void providesInstanceProviderByParameter() throws Exception {
    DependencySupplierImpl dependencySupplier = createDependencySupplier("foo", -1);
    ParameterizedType stringProviderParameterizedType =
        newParameterizedType(Provider.class, String.class);

    Object instance =
        dependencySupplier.getInstance(
            createParameter(Provider.class, stringProviderParameterizedType, null));

    assertTrue(
        String.format(
            "Instance of javax.inject.Provider expected but %s found",
  ...
  public static class SomeType {
    @Inject private String annotated;

    @MyInject private String annotatedWithCustomAnnotation;
  ...
  public void providesInstanceForInjectFieldAnnotatedWithCustomAnnotation() throws Exception {
    ...
    java.lang.reflect.Field field =
        SomeType.class.getDeclaredField("annotatedWithCustomAnnotation");
    FieldInjector fieldInjector = createFieldInjector(field);
    Object instance = dependencySupplier.getInstance(fieldInjector);
    assertEquals("foo", instance);
  }
  ...
  private DependencySupplierImpl createDependencySupplier(
      Class<? extends Annotation> annotationType, Object... instances) {
