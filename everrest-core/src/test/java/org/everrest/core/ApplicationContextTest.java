...
public class ApplicationContextTest {
    ...
    public void getsAbsolutePathBuilderAndRemovesQueryParametersAndFragment() {
        ...
        assertEquals(URI.create("http://localhost:8080/a/b/c"), applicationContext.getAbsolutePathBuilder().build());
    }
    ...
    public void getsBaseUriBuilder() {
        ...
        assertEquals(URI.create("http://localhost:8080/servlet/"), applicationContext.getBaseUriBuilder().build());
    }
    ...
    public void resolvesUriAgainstBaseUri() {
        ...
        assertEquals(URI.create("http://localhost:8080/a/b/c"), applicationContext.resolve(URI.create("c")));
    }
    ...
    public void getsMethodInvoker() {
        ...
        MethodInvoker methodInvoker = applicationContext.getMethodInvoker(mock(GenericResourceMethod.class));
        assertTrue(String.format("Expected instance of DefaultMethodInvoker but actual is %s", methodInvoker),
                   methodInvoker instanceof DefaultMethodInvoker);
    }

    @Test
    public void getsDecoratedMethodInvoker() {
        MethodInvokerDecoratorFactory methodInvokerDecoratorFactory = mock(MethodInvokerDecoratorFactory.class);
        when(methodInvokerDecoratorFactory.makeDecorator(isA(MethodInvoker.class)))
                .thenAnswer(invocation -> new MethodInvokerDecorator((MethodInvoker)invocation.getArguments()[0]));
        applicationContext = anApplicationContext()
                .withRequest(request)
                .withResponse(response)
                .withProviders(providers)
        ...
        MethodInvoker methodInvoker = applicationContext.getMethodInvoker(mock(GenericResourceMethod.class));
        assertTrue(String.format("Expected instance of MethodInvokerDecorator but actual is %s", methodInvoker),
    ...
    public void getsMethodInvokerFor_OPTIONS_Request() {
        ...
        MethodInvoker methodInvoker = applicationContext.getMethodInvoker(mock(GenericResourceMethod.class));
        assertTrue(String.format("Expected instance of OptionsRequestMethodInvoker but actual is %s", methodInvoker),
    ...
    public void getsEncodedPathSegments() {
        when(request.getBaseUri()).thenReturn(URI.create("http://localhost:8080/servlet"));
        when(request.getRequestUri()).thenReturn(URI.create("http://localhost:8080/servlet/a/%20b;x=y"));

        List<PathSegment> expectedPathSegments = newArrayList(PathSegmentImpl.fromString("a", false), PathSegmentImpl.fromString("%20b;x=y", false));
    ...
    public void getsDecodedPathSegments() {
        ...
        List<PathSegment> expectedPathSegments = newArrayList(PathSegmentImpl.fromString("a", true), PathSegmentImpl.fromString("%20b;x=y", true));
        assertEquals(expectedPathSegments, applicationContext.getPathSegments());
    }

    @Test
    public void getsEncodedQueryParameters() {
        when(request.getRequestUri()).thenReturn(URI.create("http://localhost:8080/servlet/a?a=x&b=%20y%20"));
    ...
    public void getsSimpleSecurityContextWhenCallerRequestsAsynchronousInvocation() {
        ...
        SimpleSecurityContext securityContext = (SimpleSecurityContext)applicationContext.getSecurityContext();
        assertNotNull(securityContext);
    }

    @Test
    public void getsSimpleSecurityContextWithPrincipalFromRequestWhenCallerRequestsAsynchronousInvocation() {
        when(request.getRequestUri()).thenReturn(URI.create("http://localhost:8080/a/b?async=true"));
        when(request.getRequestHeaders()).thenReturn(new MultivaluedHashMap<>());
    ...
    @Test
    public void getsSimpleSecurityContextWithPrincipalFromRequestAndRolesFromWebApplicationDeclaredRolesWhenCallerRequestsAsynchronousInvocation() {
        when(request.getRequestUri()).thenReturn(URI.create("http://localhost:8080/a/b?async=true"));
        when(request.getRequestHeaders()).thenReturn(new MultivaluedHashMap<>());
        when(request.getAuthenticationScheme()).thenReturn("BASIC_AUTH");
        ...
        Principal principal = mockPrincipal("andrew");
        when(request.getUserPrincipal()).thenReturn(principal);
        WebApplicationDeclaredRoles webApplicationDeclaredRoles = mockWebApplicationDeclaredRoles("user");
        when(environmentContext.get(WebApplicationDeclaredRoles.class)).thenReturn(webApplicationDeclaredRoles);
    ...
    private WebApplicationDeclaredRoles mockWebApplicationDeclaredRoles(String... roles) {
        WebApplicationDeclaredRoles webApplicationDeclaredRoles = mock(WebApplicationDeclaredRoles.class);
    ...
    public void notifiesLifecycleComponentWhenStopped() {
        ...
        applicationContext.getAttributes().put("org.everrest.lifecycle.PerRequest", newArrayList(lifecycleComponent));
